...
import { Question, SchoolApplication } from "api-definitions/backend";
import {
  ArrayDataResponse,
  SingleDataResponse,
...
export default async function createSchoolApplication(
  req: NextApiRequest,
  res: NextApiResponse
) {
  if (req.method !== "POST") {
  ...
  const { questionCollectionId, userId, schoolId } = req.body;
  ...
  const applicationResult = await axios.post<
    SingleDataResponse<SchoolApplication>
  >(
    ...
    try {
      ...
      await Promise.all(
        questions?.map(async (question) => {
          await createAnswer(question, application, "", token, "school");
        })
      );
    } catch (error: any) {
      console.log(error);
